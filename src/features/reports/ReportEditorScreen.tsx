...
export function ReportEditorScreen({ reportId, onClose }: ReportEditorScreenProps) {
  ...
  const loadReport = useCallback(async () => {
    try {
      ...
      const firstEditable = (data.sections || []).find((s: ReportSection) =>
        !['page de garde', 'sommaire'].includes(s.title.toLowerCase())
      );
  ...
  const handleWebViewMessage = useCallback(async (event: any) => {
    try {
      ...
      if (msg.type === 'saveSection') {
        setSaving(true);
        try {
          await authFetch(`/api/mobile/reports/${reportId}/sections/${msg.sectionId}`, {
            method: 'PATCH',
      ...
      if (msg.type === 'updateCoverData') {
        setSaving(true);
        try {
          await authFetch(`/api/mobile/reports/${reportId}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
      ...
      if (msg.type === 'aiRequest') {
        setAiLoading(true);
        try {
          const res = await authFetch('/api/mobile/reports/ai', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              action: msg.action,
      ...
      console.warn('[ReportEditor] Message parse error:', err);
    }
  }, [reportId]);
  ...
  const handleAddSection = useCallback(async () => {
    ...
    try {
      ...
      const res = await authFetch(`/api/mobile/reports/${reportId}/sections`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  }, [newSectionTitle, sections, reportId, report, sections]);
    ...
    Alert.alert(
      ...
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await authFetch(`/api/mobile/reports/${reportId}/sections/${sectionId}`, {
                method: 'DELETE',
              });
  ...
  }, [sections, currentSectionId, reportId, report]);

  // Export PDF
  const handleExportPdf = useCallback(() => {
    setExporting(true);
    Linking.openURL(`${authBaseUrl}/api/mobile/reports/${reportId}/export/pdf`)
