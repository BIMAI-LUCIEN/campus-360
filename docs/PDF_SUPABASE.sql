-- Campus-Bordes - schema Supabase PDF complet.
-- A executer dans un projet Supabase, ou via `npm.cmd run supabase:setup` dans admin-app.
...
begin
  ...
  values (new.id, new.email, coalesce(new.raw_user_meta_data->>'name', new.email, 'Etudiant Campus-Bordes'))
